...
\begin{problem}[The Quotient Category $\mathsf{hTop}$]
	...
	\begin{enumerate}[label = \textup{(}\alph*\textup{)}]
		...
		\item We say that two topological spaces $X$ and $Y$ are of the \bld{same homotopy type}, if they are isomorphic in $\mathsf{hTop}$. An explicit choice of an isomorphism is called a \bld{homotopy equivalence}. Moreover, a topological space $X$ is said to be \bld{contractible} if $\id_X$ is freely homotopic to a constant map. Show that a topological space $X$ has the same homotopy type as a one point space $\ast$ if and only if $X$ is contractible.
	\end{enumerate}	
\end{problem}
